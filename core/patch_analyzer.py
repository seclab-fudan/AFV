...
def diff_hunk_analysis(content, block, aiming="pre"):
    ...
    bur = re.findall(r'^@@ ([-+])([0-9].*?),([0-9].*?) ([-+])([0-9].*?),([0-9].*?) @@', content[slow])[0]
...
class PatchAnalyzer(object):
    def get_patch_modification_file_list(self):
        ...
        for k, v in r.items():
            ...
            if file_name.split('.')[-1] not in PHP_EXTENSION:
                continue
            self.source_code_helper.download_source(
                    git_repository=self.repository, version=self.commit_id, file_name=file_name
            )
            self.source_code_helper.download_source(
                    git_repository=self.repository, version=self.prepatch_commit_id, file_name=file_name
            )
    ...
    def find_modify_lines_from_diff(self):
        ...
        for k, v in r.items():
            ...
            if file_name.split('.')[-1] not in PHP_EXTENSION:
                continue
            elif file_name not in StorageHelper.get_patch_modified_files_in_cve(repository_name=self.repository,
                                                                                cve_id=self.cve_id):
                continue
            for _v in v:
    ...
    def __search_affected_param(self, _affect_param, file_name, lines_min, lines_max, file_id_pre):
        ...
        source_path = self.source_code_helper.download_source(git_repository=self.repository,
                                                              ...
                                                              file_name=file_name)
        assert os.path.exists(source_path), f"[*] file not found for {source_path}"
        ...
        for line in _potential_line:
            ...
            if _res is not None:
                ...
                if _res is not None and _res[NODE_TYPE] in {TYPE_ASSIGN, TYPE_ASSIGN_OP, TYPE_ASSIGN_REF}:
